...
import { HeaderIntrest } from '../intrest-sc/header-intrest';
import { useApp, usePrefetch, useUser } from 'hooks';
import { Buttons } from './buttons-area';
import api from 'api/api';
import Toast from 'react-native-toast-message';
import { ErrorComponent } from '../intrest-sc/error-componnet';
import { useCommunities } from 'hooks';
import { apiRoutes, QUERYS } from 'constants/index';

export const CommuntyScreen = () => {
  const { setCommunities, communities } = useApp();

  const queryClient = useQueryClient();
  const { setChoosedIntrest } = useUser();
